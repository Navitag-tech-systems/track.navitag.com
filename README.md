...
**IAP is iOS-only at the BUILD level, not just at runtime (2026-08-17).** `capacitor.config.json` sets `android.includePlugins` to an explicit list of the 10 Android plugins, which omits `@revenuecat/purchases-capacitor` — so `cap sync android` never wires it in and the Play Billing library is not compiled into the AAB at all. Android has no IAP and is meant to stay that way; the runtime gate (`isIapSupported()` → `getPlatform() === 'ios'`) is now a second line of defence rather than the only one. iOS is untouched and still resolves all 11 plugins (`npx cap ls` confirms 10 vs 11).
  - **`includePlugins` is an ALLOW-list that REPLACES the dependency scan — it does not filter it** (see `@capacitor/cli/dist/plugin.js` `getPlugins()`). Any plugin added to `package.json` later is therefore silently missing on Android only: the AAB still compiles and the app throws "not implemented on android" on a device. JSON has no comments, so this is enforced by **`scripts/check-android-plugins.mjs`**, which runs in CI ahead of every `cap sync android` (the `release`, `android-google-play`, and `android-production` workflows) and fails the build on both a missing new plugin and a stale list entry. Run it locally with `node scripts/check-android-plugins.mjs`.
