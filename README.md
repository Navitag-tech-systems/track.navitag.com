...
### iOS map-tile diagnostic (in progress)

iOS TestFlight builds render **blank map tiles** (works on Android). Under investigation with an on-device console rather than guesswork: the **`ios-testflight`** workflow sets `VITE_DEBUG_CONSOLE=true`, which makes Vite bundle **eruda** + `window.netcheck()` (`src/utils/debug/netcheck.js`). `netcheck` probes each host on BOTH the native layer (CapacitorHttp → URLSession, immune to `WKAppBoundDomains`) and the webview layer (`<img>` / WebSocket — the only layer app-bound domains could gate). `WKAppBoundDomains` has been collapsed to **all wildcards** — `*.navitag.net`, `*.navitag.com` (covers api, all tservers, and posbroker), `*.tile.openstreetmap.org`, `*.locationiq.com` — and `netcheck` verifies that wildcard config actually covers every host on-device (incl. the `posbroker.navitag.com` socket). Note: per Apple (WWDC20 #10188) app-bound domains gate *interaction*, not resource *loading*, so the tile cause may be elsewhere (LocationIQ key/referrer, CSP) — `netcheck` distinguishes these. The flag is set ONLY on `ios-testflight`; the `release` workflow stays clean. **Remove `VITE_DEBUG_CONSOLE` before any production release.**
