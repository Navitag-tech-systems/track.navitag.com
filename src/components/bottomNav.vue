...
const navItems = [
  ...
  { name: 'List', icon: 'fa-solid fa-list-ul', routeName: 'list-devices', tab: 'list' }, // Assuming 'list' route exists
  { name: 'Add', icon: 'fa-solid fa-plus', routeName: 'linkdevice-select', tab: 'add' },
    ...
    <div class="flex justify-around items-center h-16">
      ...
      >
        <i :class="[
          ...
          { 'scale-110': route.meta?.activeTab === item.tab }
        ]"></i>
        
        <span class="text-[10px] uppercase font-bold tracking-tighter">{{ item.name }}</span>
        
        <div v-if="route.meta?.activeTab === item.tab" 
             class="absolute top-1 w-1 h-1 bg-blue-600 rounded-full"></div>
      </button>
    </div>
  </nav>
</template>
